using LINQ.Recap.Classes;
using System;

Console.WriteLine("LINQ Recap");
Console.WriteLine();
...
15. Create new class ProductDetails with 3 properties:
  - Id, Title and Price and map the existing product data to a collection of ProductDetails objects.

*/

var smartphones = products.Where(p => p.Category == ProductCategory.Smartphones).ToList();

Console.WriteLine("We found the following products in the Smartphones category: \n");
...
var smartphoneNames =
    products.Where(p => p.Category == ProductCategory.Smartphones)
    .Select(p => p.Title).ToList();

Console.WriteLine("\nWe found the following smartphone names: \n");
foreach (var name in smartphoneNames)
{
    Console.WriteLine(name);
}

var productPriceGreaterThan1000 =
    products.Where(p => p.Price > 1000).ToList();
Console.WriteLine("\nWe found the following products with a price greater than 1000: \n");
foreach (var product in productPriceGreaterThan1000)
{
    Console.WriteLine(product);
}

var skincareProducts =
    products.Where(p => p.Category == ProductCategory.Skincare)
    .Select(p => p.Brand).ToList();
Console.WriteLine("\nWe found the following skincare product brands: \n");
foreach (var brand in skincareProducts)
{
    Console.WriteLine(brand);
}

var productRatingAbove4point8 =
    products.Where(p => p.Rating > 4.8).ToList();
Console.WriteLine("\nWe found the following products with a rating above 4.8: \n");
foreach (var product in productRatingAbove4point8)
{
    Console.WriteLine(product);
}
...
{
    Console.WriteLine("\nWe found the following products that are out of stock: \n");
...
var fragrancePrices = products.Where(p => p.Category == ProductCategory.Fragrances)
            .Select(p => p.Price).ToList();
Console.WriteLine("\nWe found the following fragrance product prices: \n");
foreach (var price in fragrancePrices)
    {
        Console.WriteLine(price);
    }

var firstProductDescription = products.Where(p => p.Rating == 4.43);
Console.WriteLine("\nWe found the following product description: \n");
foreach (var product in firstProductDescription)
{
    Console.WriteLine(product);
}

var lastSkincareProductUnder50 = products.Where(p => p.Category == ProductCategory.Skincare).Last();

Console.WriteLine($"\nWe found the following product with a price under $50: \n\n{lastSkincareProductUnder50}");

var fragranceOver100 = products.Where(p => p.Category == ProductCategory.Fragrances && p.Price > 100).First();
Console.WriteLine($"\nWe found the following product with a price over $100: \n\n{fragranceOver100}");

var brandProductStockLowerThan40 = products.Where(p => p.Stock < 40).Last();
Console.WriteLine($"\nWe found the following product with a stock lower than 40: \n\n{brandProductStockLowerThan40}");

var firstLaptopStockGreaterThan80 = products.Where(p => p.Category == ProductCategory.Laptops && p.Stock > 80).First();
Console.WriteLine($"\nWe found the following laptop with a stock greater than 80: \n\n{firstLaptopStockGreaterThan80}");

var firstProductPriceAbove1200 = products.Where(p => p.Price > 1200).First();
Console.WriteLine($"\nWe found the following product with a price above $1200: \n\n{firstProductPriceAbove1200}");

var stockCountSmartphones = products.Where(p => p.Category == ProductCategory.Smartphones)
    .Select(p => p.Stock).Last();
Console.WriteLine($"\nWe have found the stock count of the last product that belongs to the Smartphones category: \n\n{stockCountSmartphones}");
...
Console.WriteLine("\nWe have mapped the existing product data to a collection of ProductDetails objects: \n");
